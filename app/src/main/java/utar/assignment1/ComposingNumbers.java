...
public class ComposingNumbers extends AppCompatActivity {
    ...
    private void checkAnswer() {
        if (firstSelectedOption != -1 && secondSelectedOption != -1) {
            ...
            if (sum == targetNumber) {
                Toast.makeText(ComposingNumbers.this, "Correct Answer", Toast.LENGTH_SHORT).show();
                generateNumberAndAnswers();
                refreshUI();
                firstSelectedOption = -1;
                secondSelectedOption = -1;
            } else {
                Toast.makeText(ComposingNumbers.this, "Incorrect Answer . Please try again!", Toast.LENGTH_SHORT).show();
                firstSelectedOption = -1;
                secondSelectedOption = -1;
            }
            resetOptionButtonBackgrounds(); // Reset button backgrounds after checking answer
        } else {
            Toast.makeText(ComposingNumbers.this, "Select two options first!", Toast.LENGTH_SHORT).show();
        }
    }
